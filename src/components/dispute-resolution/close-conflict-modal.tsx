...
export default function CloseConflictModal({
  ...
  useEffect(() => {
    setRecipient(dispute?.parties.find((e) => e.id === dispute.userId));
  }, [dispute, onConfirm]);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-lg !rounded-sm">
        ...
        <div className="mt-2 space-y-6">
          <div className="flex items-center gap-3 p-3 overflow-hidden border rounded-sm lg:flex-row border-gray-400/1">
            <label className="block text-sm font-medium">
              {loading || !recipient ? 'Loading...' : 'Release payment to'}
            </label>
            {loading || !recipient ? (
              <Progress indeterminate={true} className="!h-2" />
            ) : (
